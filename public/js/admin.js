...
async function checkUser_AndLoad() {
  ...
  // Back navigation prevention - deeper trap and immediate redirect
  console.log('Admin authenticated, setting up back navigation trap...');
  // Push a very deep stack
  for (let i = 0; i < 50; i++) {
    window.history.pushState({ page: 'admin-dashboard', trap: true }, '', window.location.href);
  }
  // Use onpopstate for broader compatibility
  window.onpopstate = function(event) {
    console.log('Back navigation detected, redirecting immediately...');
    // Immediate redirect without pushing more states
    window.location.replace('./admin-dashboard.html'); // Use replace to avoid adding to history
  };
  // Optional: Warn on unload
  window.addEventListener('beforeunload', (event) => {
    event.preventDefault();
    event.returnValue = 'You cannot navigate back from this admin page.';
  });
